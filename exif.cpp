...
rint8u Exif::readByte(QFile &file){
    char a;
    file.getChar(&a);
    return (rint8u)a;
}

rint8u Exif::readByte(QByteArray file){
    char a;
    a = file[readIndex];
    readIndex++;
    return (rint8u)a;
...
//--------------------------------------------------------------------------
int Exif::readJpegSections(QByteArray file, int *Orientation){
    QByteArray *data;
    rint8u a;
    ...
    for(;;){
        ...
        data = new QByteArray(file.mid(readIndex,itemlen-2));
        readIndex+=itemlen-2;
        //data = new QByteArray(buffer->read(itemlen-2)); // Read the whole section.
